...
// 添加记录函数
void logPerformanceData(const char* filename, int numThreads, double serialTime, double threadTime) {
    std::ofstream file;
    file.open(filename, std::ios::app); // 打开文件，追加模式
    if (!file.is_open()) {
        fprintf(stderr, "Error: Unable to open log file %s\n", filename);
        return;
    }

    double speedup = serialTime / threadTime; // 计算加速比
    file << numThreads << " " << serialTime * 1000 << " " << threadTime * 1000 << " " << speedup << "\n";
    file.close();
}
...
int main(int argc, char** argv) {
    ...
    const unsigned int height = 1200;  //图像分辨率
    ...
    delete[] output_thread;
    // 在 main 函数的末尾添加：
    logPerformanceData("performance_log.txt", numThreads, minSerial, minThread);
    return 0;
}
